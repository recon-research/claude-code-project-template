...
- **CI posture**: &lt;light (default) · full · manual&gt; — *how much* CI runs; repo variable `CI_POSTURE` (`gh variable set CI_POSTURE --body <posture>`; unset = light). *light*: the consolidated `static gates` job per PR (~1 min); heavy matrix on demand. *full*: the heavy build/test/lint matrix too. *manual*: nothing automatic — a clean preflight pasted in the PR is the merge gate. Escalation in any posture: `full-ci` label · `gh workflow run ci.yml` · `M*`/`v*` tag. **Preflight runs every gate locally in all postures** — posture changes when CI re-verifies, never what must pass.
- **CI gating**: &lt;blocking (default) · advisory&gt; — *when* the heavy matrix runs (only meaningful in `full`); repo variable `CI_GATING` (unset = blocking). *blocking*: heavy runs on the PR and must be green to merge. *advisory*: heavy does **not** run on PRs; it runs **post-merge on the main push** as a sanity check — the PR merges on `static gates` + preflight (which runs the same heavy gates locally), and a red post-merge run is caught by the next `onboard`, which files a `bug` and fixes forward. Advisory ⇒ branch protection requires **only `static gates`** (`docs/AUTOMATION.md` §5–6).
- **CI path modularity** (always on, no config): a **doc-only** change (every changed file is `*.md` / under `docs/` / `_intake/`) never triggers the heavy matrix, in any posture — `static gates` still runs (it checks links, citations, SECTIONS freshness, hygiene). This is the "15 minutes of full CI to edit the roadmap" tax, removed.
