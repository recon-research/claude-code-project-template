...
- **Labels**: `slice` (a unit of roadmap work) · `epic` (a far milestone's container issue) · `decision` (fork awaiting the human) · `followup` (deferred work) · `idea` · `debt` · `bug` · `blocked` · `research` (frontier question to survey). Created once at repo setup (`onboard` Mode A); the issue templates apply them.
