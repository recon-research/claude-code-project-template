...
- **CI posture**: &lt;light (default) · full · manual&gt; — mirrored by the repo variable `CI_POSTURE` (`gh variable set CI_POSTURE --body <posture>`; unset = light). *light*: the consolidated `static gates` job per PR (~1 min); heavy matrix on demand. *full*: everything, every PR + main push. *manual*: nothing automatic — a clean preflight pasted in the PR is the merge gate. Escalation in any posture: `full-ci` label · `gh workflow run ci.yml` · `M*`/`v*` tag. **Preflight runs every gate locally in all postures.**
