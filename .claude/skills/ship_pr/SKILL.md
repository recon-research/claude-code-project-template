...
5. **Watch CI — per the posture** (conventions › Operating posture). `gh pr checks <n> --watch` (or poll `gh pr checks <n>`); expect **one** run per push event. *light*: `static gates` green is the gate; the heavy jobs report **skipped — that satisfies required checks and is not a failure**. Label the PR `full-ci` first when it warrants the real matrix (risky slice, milestone closer, build/test commands changed). *full*: every check green. *manual*: no checks will appear — paste the preflight PASS output into the PR body as the merge evidence. Any red check routes back to step 1 via preflight — a clean preflight with a red CI is environmental (read the log, don't guess).
