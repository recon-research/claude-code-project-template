...
2. Fill [`CLAUDE.md`](../../../CLAUDE.md): the project name, one-liner, and the **Status** block. Remove the "new project" blockquote. **Stamp provenance:** fill `TEMPLATE_VERSION` (template source path/URL · `git -C <source> rev-parse HEAD` · today) — [`update_from_template`](../update_from_template/SKILL.md) depends on this stamp; if the source isn't reachable, record what's known.
