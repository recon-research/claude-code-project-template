...
1. **Preflight (cheap, mechanical):** `gh auth status` works; `git status` clean; **main CI is green** (`gh run list --branch main -L 1`). Read the CI posture (conventions › Operating posture) before judging: in `light` a main-push run reports conclusion **`skipped`** (its jobs were posture-skipped — verified live 2026-06-12), and in `manual` there may be no runs at all — neither is red; judge by the last PR's `static gates` / pasted preflight instead. Edge cases, each with a safe default — never improvise here:
